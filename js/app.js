...
function uploadAmsatStatus(data) {
    ...
    // Parse date from the time string
    const dateObj = new Date(data.timeOn);
    
    // Calculate the 15-minute period (0-3) from the minutes
    const period = Math.floor((dateObj.getMinutes() - 1) / 15);
    ...
    return fetch('/api/amsat_status_proxy.php', {
        ...
        body: JSON.stringify({
            satName: satName,
            status: data.status, // Can be Heard or Not Heard
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error('Failed to submit to AMSAT Status: ' + response.status);
        }
        return response.text();
    });
}
