...
function generateScheduleTable() {
    ...
    if (allScheduledPasses.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="7" style="text-align: center;">No passes found in the next ${days} day(s)</td></tr>`;
        return;
    }
...
function filterScheduleTable() {
    ...
    filteredPasses.forEach(pass => {
        ...
        const calendarLink = createScheduleGoogleCalendarLink(pass);
    ...
    if (filteredPasses.length === 0) {
        const row = document.createElement('tr');
        row.innerHTML = `<td colspan="7" style="text-align: center;">No passes found for ${satelliteFilter}</td>`;
        tableBody.appendChild(row);
    }
}
...
function createScheduleGoogleCalendarLink(pass) {
    // Format dates in the required format: YYYYMMDDTHHmmssZ
    const formatDateForCalendar = (date) => {
        return date.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
    };
    ...
    // Create event title and description
    const title = `Satellite Pass: ${pass.satellite}`;
    const description = `Satellite pass details:\n` +
